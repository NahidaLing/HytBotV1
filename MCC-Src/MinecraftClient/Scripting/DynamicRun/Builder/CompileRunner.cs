...
namespace MinecraftClient.Scripting.DynamicRun.Builder
{
    ...
    {
        public object? Execute(byte[] compiledAssembly, string[] args, ChatBot apiHandler)
        {
            var assemblyLoadContextWeakRef = LoadAndExecute(compiledAssembly, args, apiHandler);
        ...
        private static Tuple<WeakReference, object?> LoadAndExecute(byte[] compiledAssembly, string[] args, ChatBot apiHandler)
        {
            ...
            var execResult = compiledScript.GetType().GetMethod("__run")!.Invoke(compiledScript, new object[] { new CSharpAPI(apiHandler), args });
