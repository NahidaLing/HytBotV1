    ...
    public class McClient : IMinecraftComHandler
    {
        ...
        public McClient(SessionToken session, PlayerKeyPair? playerKeyPair, string server_ip, ushort port, int protocolversion, ForgeInfo? forgeInfo)
        {
            ...
            Log = Settings.Config.Logging.LogToFile
                ? new FileLogLogger(Settings.Config.Logging.LogFile, Settings.Config.Logging.PrependTimestamp)
                : new FilteredLogger();
            ...
            else
            {
                ...
                else if (text.Length > 0)
                {
                        ...
                        || text[0] == Config.Main.Advanced.InternalCmdChar.ToChar())
                    {
                        ...
                        if (!PerformInternalCommand(command, ref result) && Config.Main.Advanced.InternalCmdChar.ToChar() == '/')
                        {
                            SendText(text);
                        }
        ...
        public bool PerformInternalCommand(string command, ref CmdResult result)
        {
        ...
        public bool DigBlock(Location location, bool swingArms = true, bool lookAtBlock = true, double duration = 0)
        {
            ...
            lock (DigLock)
            {
                ...
                handler.SendPlayerDigging(0, location, blockFace, sequenceId++); 
                if(!swingArms) { DoAnimation((int)Hand.MainHand);}
                handler.SendPlayerDigging(1, location, blockFace, sequenceId++);
                return true;
            }
        }
