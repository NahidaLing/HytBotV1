...
#pragma warning restore format // @formatter:on
        ...
        private int DigAt(CmdResult r, Location blockToBreak, double duration = 0)
        {
            ...
            if (handler.DigBlock(blockToBreak, duration: duration))
            {
                blockToBreak = blockToBreak.ToCenter();
                return r.SetAndReturn(Status.Done, string.Format(Translations.cmd_dig_dig, blockToBreak.X, blockToBreak.Y, blockToBreak.Z, ""));
            }
            else
